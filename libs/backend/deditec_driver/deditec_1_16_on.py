...
# Import loggeru z našeho dummy common
try:
    from backend.common import get_logger, handle_timeout
except ImportError:
     # Fallback, pokud by common nebylo dostupné (nemělo by nastat)
     import logging
     logging.basicConfig()
     get_logger = logging.getLogger
     def handle_timeout(signum, frame): raise TimeoutError("Timeout")

logger = get_logger(__name__) # Použije logger nakonfigurovaný v main
...
class Deditec_1_16_on:
    def __init__(self, ip: str = IP, port: int = PORT, timeout_seconds: int = 3):
        ...
        logger.debug(f"Deditec communication object created for {self.ip}:{self.port}")

    def connect(self) -> bool: # Vrací bool pro úspěch/neúspěch
        if self.socket is not None:
             logger.warning("Deditec:: connect called, but socket already exists. Closing first.")
             self.close_connection()

        logger.debug(f"Deditec:: connecting to device at {self.ip}:{self.port}...")
        try:
            self.socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
            # Nastavení timeoutu pro operace socketu (connect, send, recv)
            self.socket.settimeout(self.timeout_seconds)
            self.socket.connect((self.ip, self.port))
            logger.debug("Deditec:: connection established")
            return True
        except socket.timeout:
            logger.error(f"Deditec:: connection timed out ({self.timeout_seconds}s)")
            self.socket = None # Zajistit, že socket je None při chybě
            return False
        except Exception as e:
            logger.exception(f"Deditec:: error when connecting: {e}")
            self.socket = None
            return False

    def send_command(self, command: bytes) -> int: # Vrací 0 pro úspěch, >0 pro chybu
        if self.socket is None:
            logger.error("Deditec:: send_command called but not connected.")
            return 1 # Chyba - není připojeno

        logger.debug(f"Deditec:: sending command: {command!r}")
        try:
            ...
            logger.debug(f"Deditec:: received confirmation data (len={len(data)}): {data!r}")
            return 0 # Předpokládáme úspěch, pokud sendall a recv nehodily výjimku
        except socket.timeout:
             logger.error(f"Deditec:: socket timeout during send/recv ({self.timeout_seconds}s)")
             return 2 # Chyba - timeout
        except Exception as e:
            logger.exception(f"Deditec:: error sending command or receiving confirmation: {e}")
            return 3 # Jiná chyba

    def close_connection(self) -> None:
        if self.socket:
            logger.debug("Deditec:: closing connection")
            try:
                self.socket.close()
            except Exception as e:
                 logger.error(f"Deditec:: error closing socket: {e}")
            finally:
                 self.socket = None # Vždy nastavit na None
        else:
             logger.debug("Deditec:: close_connection called but already closed.")
    ...
    def __enter__(self) -> Self:
        ...
        try:
            ...
            signal.alarm(self.timeout_seconds + 1) # Dáme o 1s víc než socket timeout
             ...
             logger.warning("Cannot set SIGALRM handler (not on Unix main thread?), relying on socket timeout.")
    ...
    def __exit__(self, exc_type: Any, exc_val: Any, exc_tb: Any) -> bool:
        ...
        if exc_type: # Pokud došlo k výjimce uvnitř 'with' bloku
            logger.error(f"Deditec:: An error occurred during 'with' block: {exc_type.__name__}: {exc_val}")
